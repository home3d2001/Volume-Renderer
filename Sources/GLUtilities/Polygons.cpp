...
void Polygons::movePolygon(int index, glm::mat4 matrix) {
	...
	error |= multiplyM4V2_p(pointsBuffer, pointsBuffer, numPtsToMove, offset, matrix);
...
void Polygons::writeToFile(const std::string &foldername) {
	using namespace std;
	ofstream out;
	ofstream subout;
	...
	for (int i = 0; i < lasts.size(); ++i) {
		...
		subout.clear();
		int last = (i + 1 != lasts.size()) ? lasts[i + 1] : points.size();
		first = lasts[i];
