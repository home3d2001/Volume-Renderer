...
#ifdef linux
//todo: add required headers for linux gl context aquisition
#elif defined _WIN32
#define NOMINMAX
#include <Windows.h>
#elif defined TARGET_OS_MAC
//todo: add required headers for mac os gl context aquisition
#endif
...
private:
	...
	static std::string CLFW::loadFile(std::string name);
...
//	static cl::DeviceCommandQueue DeviceQueue;
	...
	static cl_context_properties ContextProperties[7];
  ...
  static cl_int getBuffer(cl::Buffer &buffer, std::string key, cl_ulong size, bool &old = lastBufferOld, bool resize = false, int flag = CL_MEM_READ_WRITE);
	...
	static void CLFW::getContextProperties();
	template<typename T>
